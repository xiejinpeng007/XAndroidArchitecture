...
import xiejinpeng.xandroidarch.ui.base.BaseLifecycleFragment
import io.reactivex.Observable
import io.reactivex.subjects.PublishSubject
import io.reactivex.subjects.Subject

/**
 * wrap Activity / Fragment Lifecycle into Observable
 */
...
sealed class FragmentLifeEvent {
    data class OnCreate(val fragment: BaseLifecycleFragment, val savedInstanceState: Bundle?): FragmentLifeEvent()

    data class OnCreateView(val fragment: BaseLifecycleFragment,
                            val inflater: LayoutInflater,
                            val container: ViewGroup?,
                            val savedInstanceState: Bundle?): FragmentLifeEvent()

    data class OnViewCreated(val fragment: BaseLifecycleFragment, val view: View?, val savedInstanceState: Bundle?): FragmentLifeEvent()
    data class OnStart(val fragment: BaseLifecycleFragment) : FragmentLifeEvent()
    data class OnResume(val fragment: BaseLifecycleFragment) : FragmentLifeEvent()
    data class OnShow(val fragment: BaseLifecycleFragment, val showMode: OnShowMode): FragmentLifeEvent()
    data class OnHide(val fragment: BaseLifecycleFragment, val hideMode: OnHideMode): FragmentLifeEvent()
    data class OnDestroy(val fragment: BaseLifecycleFragment): FragmentLifeEvent()
}
