...
import xiejinpeng.xandroidarch.BuildConfig
import xiejinpeng.xandroidarch.manager.api.auth.ApiKeyAuth
import xiejinpeng.xandroidarch.manager.api.auth.HttpBasicAuth
import xiejinpeng.xandroidarch.manager.api.auth.OAuth
            ...
            get() {
                for (apiAuthorization in apiAuthorizations.values) {
                    if (apiAuthorization is OAuth) {
                        ...
                        return oauth.tokenRequestBuilder
                    }
                }
                return null
            }
        ...
        val authorizationEndPoint: OAuthClientRequest.AuthenticationRequestBuilder?
            get() {
                for (apiAuthorization in apiAuthorizations.values) {
                    if (apiAuthorization is OAuth) {
                        val oauth = apiAuthorization
                        return oauth.authenticationRequestBuilder
                    }
                }
                return null
            }
        ...
        private fun setCredentials(username: String, password: String): Builder {
            for (apiAuthorization in apiAuthorizations.values) {
                if (apiAuthorization is HttpBasicAuth) {
                    val basicAuth = apiAuthorization
                    basicAuth.setCredentials(username, password)
                    break
                }
                if (apiAuthorization is OAuth) {
                    val oauth = apiAuthorization
                    oauth.tokenRequestBuilder?.setUsername(username)?.setPassword(password)
                    break
                }
            }
            return this
